...
@dataclass
class Int4BlockKVCache(BlockKVCache):
    """Block K/V cache backed by groupwise packed INT4 tensors.

    The packed tensors may live on CPU while attention runs on CUDA. Cache reads
    transfer and dequantize only the visible history for the current layer. The
    current chunk remains in the model's activation dtype and is committed by
    :meth:`update` only when the caller has a finalized chunk.
    """

    group_size: int = 32
    """Number of adjacent head-dimension values sharing one FP16 scale."""

    storage_device: torch.device | str = torch.device("cpu")
    """Device holding packed values and scales between attention calls."""

    commit_current: bool = False
    """Whether the next attention call should persist its current chunk."""

    _k_scale: Tensor = field(init=False)
    """Per-group key scales in FP16."""

    _v_scale: Tensor = field(init=False)
    """Per-group value scales in FP16."""
    ...
    def __post_init__(self) -> None:
        ...
        self.seq_dim = self.seq_dim if self.seq_dim >= 0 else self.seq_dim + tensor_dim
        assert self.seq_dim != tensor_dim - 1, (
            "INT4 cache sequence dimension must not be the quantized last dimension"
        )
        assert self.sink_size >= 0, "sink_size must be non-negative"
        assert self.group_size > 0 and self.group_size % 2 == 0, (
            "group_size must be a positive even integer"
        )
        ...
        for name, shape in (("k", self.k_shape), ("v", self.v_shape)):
            assert shape[-1] % self.group_size == 0, (
                f"{name}_shape[-1] ({shape[-1]}) must be divisible by "
                f"group_size ({self.group_size})"
            )

        storage_device = torch.device(self.storage_device)
        self.storage_device = storage_device
        self.device = torch.device(self.device)
        self._k = torch.empty(
            (*self.k_shape[:-1], self.k_shape[-1] // 2),
            device=storage_device,
            dtype=torch.uint8,
        )
        self._v = torch.empty(
            (*self.v_shape[:-1], self.v_shape[-1] // 2),
            device=storage_device,
            dtype=torch.uint8,
        )
        self._k_scale = torch.empty(
            (*self.k_shape[:-1], self.k_shape[-1] // self.group_size),
            device=storage_device,
            dtype=torch.float16,
        )
        self._v_scale = torch.empty(
            (*self.v_shape[:-1], self.v_shape[-1] // self.group_size),
            device=storage_device,
            dtype=torch.float16,
        )
    ...
    def storage_nbytes(self) -> int:
        """Bytes reserved by packed K/V values and their scales."""
        return sum(
            self._tensor_nbytes(tensor)
            for tensor in (self._k, self._v, self._k_scale, self._v_scale)
        )

    @property
    def uncompressed_nbytes(self) -> int:
        """Bytes the same K/V shapes require in the configured model dtype."""
        element_size = torch.empty((), dtype=self.dtype).element_size()
        return (
            torch.Size(self.k_shape).numel() + torch.Size(self.v_shape).numel()
        ) * element_size

    def _quantize(self, tensor: Tensor) -> tuple[Tensor, Tensor]:
        grouped = tensor.reshape(*tensor.shape[:-1], -1, self.group_size)
        max_abs = grouped.abs().amax(dim=-1, keepdim=True)
        scale = torch.where(max_abs > 0, max_abs / 7, torch.ones_like(max_abs))
        codes = (torch.round(grouped / scale).clamp(-7, 7) + 8).to(torch.uint8)
    ...
    def _dequantize(
        ...
        codes = torch.empty(
            (*packed.shape[:-1], last_dim),
            device=self.device,
            dtype=torch.uint8,
        )
        codes[..., 0::2] = packed & 0x0F
        codes[..., 1::2] = packed >> 4
        values = codes.to(self.dtype).reshape(*codes.shape[:-1], -1, self.group_size)
        values = (values - 8) * scale.to(self.dtype).unsqueeze(-1)
        return values.flatten(-2)

    def _roll_local_window_left(self, shift_size: int) -> None:
        """Shift packed values and scales together within the local window."""
        total_size = self._k.shape[self.seq_dim]
        assert 0 < shift_size <= self.chunk_size, (
            f"shift_size ({shift_size}) must be in (0, {self.chunk_size}]"
        )
        ...
        if tokens_to_keep > 0:
            ...
            src_slice = self._seq_slice(src_start, src_end)
            for tensor in (self._k, self._v, self._k_scale, self._v_scale):
                tensor[dst_slice] = tensor[src_slice].clone()
        self._n_cached = total_size

    def _write_current_chunk(self, k: Tensor, v: Tensor) -> None:
        """Quantize and store the current finalized chunk."""
        write_start, write_end = self._current_write_bounds()
        read_start = 0
        read_end = write_end - write_start
        if (
        ...
        k_packed, k_scale = self._quantize(k)
        v_packed, v_scale = self._quantize(v)
        sl_read = self._seq_slice(read_start, read_end)
        sl_write = self._seq_slice(write_start, write_end)
        for destination, source in (
            ...
            destination[sl_write].copy_(source[sl_read].to(self.storage_device))

    @property
    def history_size(self) -> int:
        """Number of committed tokens preceding the in-progress chunk."""
        if self._curr_chunk_idx is None:
            return self._n_cached
        write_start, _ = self._current_write_bounds()
        return write_start

    def cached_history(self) -> tuple[Tensor, Tensor] | None:
        """Dequantize committed history, excluding the in-progress chunk."""
        if self.history_size == 0:
            return None
        cache_slice = self._seq_slice(0, self.history_size)
        return (
            self._dequantize(
                self._k[cache_slice], self._k_scale[cache_slice], self.k_shape[-1]
            ),
            self._dequantize(
                self._v[cache_slice], self._v_scale[cache_slice], self.v_shape[-1]
            ),
        )

    def cached_k(self) -> Tensor:
        """Return the visible key prefix dequantized to the compute device."""
        cache_slice = self._seq_slice(0, self._visible_end())
        return self._dequantize(
            self._k[cache_slice], self._k_scale[cache_slice], self.k_shape[-1]
        )

    def cached_v(self) -> Tensor:
        """Return the visible value prefix dequantized to the compute device."""
        cache_slice = self._seq_slice(0, self._visible_end())
        return self._dequantize(
            self._v[cache_slice], self._v_scale[cache_slice], self.v_shape[-1]
        )
    ...
    def reset(self) -> None:
        """Reset cache bookkeeping and leave commit mode disabled."""
        super().reset()
